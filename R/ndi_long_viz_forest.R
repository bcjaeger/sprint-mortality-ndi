...
#' @title
#' @param long_inf
ndi_long_viz_mort <- function(long_inf, recoders) {
...
ndi_long_viz_cvd <- function(long_inf, recoders) {
  ...
  data_init <- long_inf |>
    mutate(variable = str_remove(variable, 'long_inf_'),
           variable = recode(variable, !!!recoders$variable),
           group = recode(group, !!!recoders$group),
           cvd_lwr = if_else(cvd_lwr < 0.10, true = cvd_est, false = cvd_lwr))
  ...
  range_y <- range(data_gg$cvd_lwr, data_gg$cvd_upr, na.rm = TRUE)
  ...
  fig_hrs <- ggplot(data_gg) +
    ...
    theme(text = element_text(family = 'serif', size = 12),
          ...
          axis.title.y = element_blank()) +
    facet_wrap(~trial_phase) +
    labs(y = 'Hazard ratio for all-cause CVD, intensive versus standard treatment') +
    scale_y_log10()
